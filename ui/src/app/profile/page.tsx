...
export default function ProfilePage() {
  ...
  // Form state - Professional (provider only)
  ...
  return (
    <div className="bg-[var(--surface)] text-[var(--on-surface)] min-h-screen pt-16 pb-24 md:pb-8">
      {/* Top App Bar */}
      <header className="fixed top-0 w-full z-50 border-b-2 border-[var(--on-surface)] bg-[var(--surface-container-lowest)]/80 backdrop-blur-md">
        <div className="flex justify-between items-center px-5 h-16 w-full max-w-screen-xl mx-auto">
          <Link href="/" className="text-xl font-black text-[var(--on-surface)] tracking-tighter uppercase">
            Nexum
          </Link>
          <button className="text-[var(--on-surface)] hover:bg-[var(--surface-container)] p-2 rounded-full transition-colors">
            <Icon name="settings" />
          </button>
        </div>
      </header>

      <main className="max-w-screen-xl mx-auto px-5 pt-6 space-y-6">
        {/* Profile Header */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Professional Info Card (Provider Only) */}
          {isProvider && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedCategoryId && availableSubcategories.length > selectedSubcategoryIds.length && (
                      <div className="relative group">
                        ...
                        {/* Dropdown for adding subcategories */}
          ...
          {/* Empty space for clients to keep layout consistent */}
          {!isProvider && <div className="hidden md:block" />}
        </div>
      ...
      <nav className="fixed bottom-0 w-full z-50 border-t-2 border-[var(--on-surface)] bg-[var(--surface-container-lowest)]/90 backdrop-blur-lg md:hidden">
        <div className="flex justify-around items-center px-4 py-3">
          <Link
            href={isProvider ? "/provider" : "/client"}
            className="flex flex-col items-center justify-center text-[var(--outline)] p-2 hover:text-[var(--primary)] transition-colors"
          >
            <Icon name="home" />
            <span className="font-mono text-[10px] uppercase mt-1">Inicio</span>
          </Link>
          <Link
            href={isProvider ? "/provider/opportunities" : "/client/requests"}
            className="flex flex-col items-center justify-center text-[var(--outline)] p-2 hover:text-[var(--primary)] transition-colors"
          >
            <Icon name={isProvider ? "work" : "receipt_long"} />
            <span className="font-mono text-[10px] uppercase mt-1">{isProvider ? "Trabajos" : "Pedidos"}</span>
          </Link>
          <Link
            href="/messages"
            className="flex flex-col items-center justify-center text-[var(--outline)] p-2 hover:text-[var(--primary)] transition-colors"
          >
            ...
            <span className="font-mono text-[10px] uppercase mt-1">Chat</span>
          </Link>
          <Link
            href="/profile"
            className="flex flex-col items-center justify-center bg-[var(--primary-container)] text-[var(--primary)] rounded-xl p-2 border-2 border-[var(--on-surface)] shadow-[2px_2px_0px_0px_var(--on-surface)]"
          >
            <Icon name="person" fill />
            <span className="font-mono text-[10px] uppercase mt-1 font-bold">Perfil</span>
          </Link>
        </div>
      </nav>
    </div>
